...
/*fn reset_data_pass(data: &RenderData, camera_matrix: &Matrix4f, device: &Device, photon_buffer: &wgpu::Buffer) -> wgpu::CommandEncoder {
    // ------------------ Uniforms ------------------ //
...

    {
        ...
        render_pass.set_pipeline(&render_pipeline);
        render_pass.set_bind_group(0, &uniforms_bind_group, &[]);
        render_pass.set_bind_group(1, &photon_bind_group, &[]);
        render_pass.draw(0..4, 0..1);
    }
...
fn render_data_pass(data: &RenderData, camera_matrix: &Matrix4f, device: &Device, photon_buffer: &wgpu::Buffer, textures: [&wgpu::Texture; 2]) -> wgpu::CommandEncoder {
    // ------------------ Uniforms ------------------ //
...
pub async fn render(data: RenderData, camera_matrix: Matrix4f, device: &Device, queue: &wgpu::Queue, output: &mut Vec<u8>) {
    let pixel_amount = data.output_resolution * data.output_resolution;
    ...
    let mut reset_encoder = reset_data_pass(&data, &camera_matrix, &device, &photon_buffer);
    ...
    reset_encoder.copy_buffer_to_buffer(
        &photon_buffer,
        0,
        &output_buffer,
        0,
    ...
    let main_encoder = render_data_pass(&data, &camera_matrix, &device, &photon_buffer, [&volume_texture, &tf_texture]);

    queue.submit([
        reset_encoder.finish(),
        main_encoder.finish()
    ]);
